// Code generated by csilgen. DO NOT EDIT.
// Source: <csil spec>
// Target: typescript-typesonly

export type HouseID = string;

export type MemberID = string;

export type RoleID = string;

export type SkillID = string;

export type GroupID = string;

export type ProjectID = string;

export type EventID = string;

export type TaskID = string;

export type CommentID = string;

export type ShareID = string;

export type TrustedDomainID = string;

export type MemberAuditID = string;

export type MilestoneID = string;

export type NotificationID = string;

export type NotificationEventID = string;

export type Timestamp = string;

export type TaskStatus = "open" | "in_progress" | "done" | "cancelled";

export type TargetType = "event" | "task" | "project";

export type AccessLevel = "none" | "read" | "edit" | "full";

export type GranteeType = "member" | "group";

export type ResourceType = "event" | "task" | "house";

export type ProjectStatus = "active" | "archived";

export type DependencyNodeType = "task" | "project";

export type RecurrenceFreq = "hourly" | "daily" | "weekly" | "monthly" | "quarterly" | "yearly";

export type MilestoneState = "done" | "current" | "future";

export interface House {
  houseId: HouseID;
  name: string;
  description?: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface Member {
  memberId: MemberID;
  houseId: HouseID;
  linkkeysDomain: string;
  linkkeysUserId: string;
  displayName?: string;
  email?: string;
  avatarUrl?: string;
  cachedPublicKey?: Uint8Array;
  createdAt: Timestamp;
  updatedAt: Timestamp;
  lastSeenAt?: Timestamp;
  deactivatedAt?: Timestamp;
}

export interface TrustedDomain {
  trustedDomainId: TrustedDomainID;
  houseId: HouseID;
  domain: string;
  createdAt: Timestamp;
}

export interface Role {
  roleId: RoleID;
  houseId: HouseID;
  name: string;
  description?: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface MemberRole {
  memberId: MemberID;
  roleId: RoleID;
  createdAt: Timestamp;
}

export interface MemberAudit {
  auditId: MemberAuditID;
  houseId: HouseID;
  subjectMemberId: MemberID;
  actorMemberId?: MemberID;
  action: string;
  targetType?: string;
  targetId?: string;
  detail?: string;
  createdAt: Timestamp;
}

export interface Skill {
  skillId: SkillID;
  houseId: HouseID;
  name: string;
  description?: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface MemberSkill {
  memberId: MemberID;
  skillId: SkillID;
  createdAt: Timestamp;
}

export interface GroupSkill {
  groupId: GroupID;
  skillId: SkillID;
  createdAt: Timestamp;
}

export interface Group {
  groupId: GroupID;
  houseId: HouseID;
  name: string;
  description?: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface GroupMember {
  groupId: GroupID;
  memberId: MemberID;
  createdAt: Timestamp;
}

export interface Project {
  projectId: ProjectID;
  houseId: HouseID;
  name: string;
  description?: string;
  category?: string;
  status?: ProjectStatus;
  visibility?: AccessLevel;
  createdByMemberId?: MemberID;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface ProjectTask {
  projectId: ProjectID;
  taskId: TaskID;
  position: number;
  createdAt: Timestamp;
}

export interface ProjectMember {
  projectId: ProjectID;
  memberId: MemberID;
  createdAt: Timestamp;
}

export interface ProjectOwner {
  projectId: ProjectID;
  memberId: MemberID;
  createdAt: Timestamp;
}

export interface Milestone {
  milestoneId: MilestoneID;
  projectId: ProjectID;
  label: string;
  whenLabel: string;
  state: MilestoneState;
  position: number;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface Event {
  eventId: EventID;
  houseId: HouseID;
  ownerMemberId: MemberID;
  title: string;
  description?: string;
  location?: string;
  startsAt?: Timestamp;
  endsAt?: Timestamp;
  allDay?: boolean;
  recurrenceFreq?: RecurrenceFreq;
  recurrenceInterval?: number;
  recurrenceByWeekday?: number[];
  recurrenceBySetpos?: number;
  nextRecurrenceAt?: Timestamp;
  recurrenceRootEventId?: EventID;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface Task {
  taskId: TaskID;
  houseId: HouseID;
  ownerMemberId: MemberID;
  assignees?: MemberID[];
  assignedToSkillId?: SkillID;
  parentTaskId?: TaskID;
  visibility?: AccessLevel;
  title: string;
  description?: string;
  status?: TaskStatus;
  dueAt?: Timestamp;
  tag?: string;
  estimateMinutes?: number;
  recurrenceFreq?: RecurrenceFreq;
  recurrenceInterval?: number;
  recurrenceByWeekday?: number[];
  recurrenceBySetpos?: number;
  nextRecurrenceAt?: Timestamp;
  recurrenceRootTaskId?: TaskID;
  deletedAt?: Timestamp;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface Comment {
  commentId: CommentID;
  houseId: HouseID;
  memberId: MemberID;
  targetType: TargetType;
  targetId: string;
  body: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

export interface Share {
  shareId: ShareID;
  houseId: HouseID;
  sharedBy: MemberID;
  linkkeysDomain: string;
  linkkeysUserId: string;
  resourceType: ResourceType;
  resourceId: string;
  accessLevel?: AccessLevel;
  createdAt: Timestamp;
  expiresAt?: Timestamp;
}

export interface HouseSummary {
  houseId: HouseID;
  name: string;
  memberId: MemberID;
  roles: string[];
}

export interface HouseRoles {
  house: HouseID;
  member: MemberID;
  roles: string[];
}

export interface Identity {
  domain: string;
  userId: string;
  displayName?: string;
  houses: HouseRoles[];
  iat: number;
  exp: number;
}

export interface LoginRequest {
  signedAssertion: string;
}

export interface CompleteRequest {
  encryptedToken: string;
}

export interface LoginResponse {
  token: string;
  domain: string;
  userId: string;
  displayName?: string;
  expiresAt: Timestamp;
}

export interface DevUserEntry {
  memberId: MemberID;
  houseId: HouseID;
  houseName: string;
  displayName?: string;
  linkkeysDomain?: string;
  linkkeysUserId?: string;
  roles: string[];
}

export interface DevUsersResponse {
  users: DevUserEntry[];
}

export interface DevLoginRequest {
  memberId: MemberID;
}

export interface MeResponse {
  domain: string;
  userId: string;
  displayName?: string;
  expiresAt: Timestamp;
  houses: HouseSummary[];
}

export interface EmptyRequest {
}

export interface EmptyResponse {
}

export interface BoolResponse {
  value: boolean;
}

export interface HouseListRequest {
  limit?: number;
  offset?: number;
}

export interface HouseScopedListRequest {
  houseId: HouseID;
  limit?: number;
  offset?: number;
}

export interface TaskList {
  tasks: Task[];
  hiddenCount: number;
}

export interface ProjectList {
  projects: Project[];
  hiddenCount: number;
}

export interface MemberScopedListRequest {
  houseId: HouseID;
  memberId: MemberID;
  limit?: number;
  offset?: number;
}

export interface ProjectScopedListRequest {
  houseId: HouseID;
  projectId: ProjectID;
  limit?: number;
  offset?: number;
}

export interface CommentListRequest {
  targetType: TargetType;
  targetId: string;
  limit?: number;
  offset?: number;
}

export interface Notification {
  notificationId: NotificationID;
  houseId: HouseID;
  memberId: MemberID;
  kind: string;
  actorMemberId?: MemberID;
  actorName: string;
  targetType?: string;
  targetId?: string;
  targetTitle: string;
  body: string;
  read: boolean;
  readAt?: Timestamp;
  createdAt: Timestamp;
}

export interface NotificationListRequest {
  houseId: HouseID;
  unreadOnly?: boolean;
  limit?: number;
  offset?: number;
}

export interface NotificationUnreadCount {
  count: number;
}

export interface ShareAccessRequest {
  linkkeysDomain: string;
  linkkeysUserId: string;
  resourceType: ResourceType;
  resourceId: string;
}

export interface ResourceRef {
  resourceType: ResourceType;
  resourceId: string;
}

export interface MemberRoleRef {
  memberId: MemberID;
  roleId: RoleID;
}

export interface MemberSkillRef {
  memberId: MemberID;
  skillId: SkillID;
}

export interface GroupSkillRef {
  groupId: GroupID;
  skillId: SkillID;
}

export interface GroupMemberRef {
  groupId: GroupID;
  memberId: MemberID;
}

export interface ProjectTaskRef {
  projectId: ProjectID;
  taskId: TaskID;
}

export interface ProjectTaskOrderRequest {
  projectId: ProjectID;
  taskId: TaskID;
  position: number;
}

export interface ProjectMemberRef {
  projectId: ProjectID;
  memberId: MemberID;
}

export interface ProjectOwnerRef {
  projectId: ProjectID;
  memberId: MemberID;
}

export interface DependencyRef {
  dependentType: DependencyNodeType;
  dependentId: string;
  dependencyType: DependencyNodeType;
  dependencyId: string;
}

export interface DependencyTarget {
  type: DependencyNodeType;
  id: string;
}

export interface DependencyNode {
  type: DependencyNodeType;
  id: string;
  title: string;
  status?: string;
}

export interface DependencyGraph {
  dependencies: DependencyNode[];
  dependents: DependencyNode[];
}

export interface Grant {
  granteeType: GranteeType;
  granteeId: string;
  accessLevel: AccessLevel;
}

export interface TaskGrantRef {
  taskId: TaskID;
  granteeType: GranteeType;
  granteeId: string;
}

export interface PutTaskGrantRequest {
  taskId: TaskID;
  granteeType: GranteeType;
  granteeId: string;
  accessLevel: AccessLevel;
}

export interface SetTaskVisibilityRequest {
  taskId: TaskID;
  visibility: AccessLevel;
}

export interface ProjectGrantRef {
  projectId: ProjectID;
  granteeType: GranteeType;
  granteeId: string;
}

export interface PutProjectGrantRequest {
  projectId: ProjectID;
  granteeType: GranteeType;
  granteeId: string;
  accessLevel: AccessLevel;
}

export interface SetProjectVisibilityRequest {
  projectId: ProjectID;
  visibility: AccessLevel;
}

export interface EffectiveSettings {
  bugReportsEnabled?: boolean;
  bugReportsProjectId?: ProjectID;
  defaultProjectVisibility?: AccessLevel;
}

export interface UpdateSettingsRequest {
  houseId: HouseID;
  settings: EffectiveSettings;
}

export interface BugReportRequest {
  houseId: HouseID;
  title: string;
  description?: string;
}

export interface ServiceError {
  code: number;
  message: string;
}

export type AuditID = string;

export interface AuditEntry {
  auditId: AuditID;
  houseId?: HouseID;
  actorMemberId?: MemberID;
  actorDomain: string;
  actorUserId: string;
  serviceName: string;
  method: string;
  action: string;
  resourceType?: string;
  resourceId?: string;
  outcome: string;
  before?: string;
  after?: string;
  detail?: string;
  createdAt: Timestamp;
}

export interface AuditQuery {
  houseId: HouseID;
  actorMemberId?: MemberID;
  resourceType?: string;
  action?: string;
  since?: Timestamp;
  until?: Timestamp;
  cursor?: string;
  limit?: number;
}

export interface AuditPage {
  entries: AuditEntry[];
  nextCursor?: string;
}

export interface TrashItem {
  resourceType: string;
  resourceId: string;
  houseId: HouseID;
  title?: string;
  deletedAt: Timestamp;
  deletedByMemberId?: MemberID;
  deletedOpId: string;
}

export interface TrashPage {
  items: TrashItem[];
  nextCursor?: string;
}

export interface RestoreRequest {
  houseId: HouseID;
  deletedOpId?: string;
  resourceType?: string;
  resourceId?: string;
}

export interface PurgeRequest {
  houseId: HouseID;
  resourceType: string;
  resourceId: string;
}

export function validateHouse(value: House): string[] {
  const errors: string[] = [];
  if (value.name.length < 1 || value.name.length > 256) errors.push("name: length must be between 1 and 256");
  return errors;
}

export function validateRole(value: Role): string[] {
  const errors: string[] = [];
  if (value.name.length < 1 || value.name.length > 128) errors.push("name: length must be between 1 and 128");
  return errors;
}

export function validateSkill(value: Skill): string[] {
  const errors: string[] = [];
  if (value.name.length < 1 || value.name.length > 256) errors.push("name: length must be between 1 and 256");
  return errors;
}

export function validateGroup(value: Group): string[] {
  const errors: string[] = [];
  if (value.name.length < 1 || value.name.length > 256) errors.push("name: length must be between 1 and 256");
  return errors;
}

export function validateProject(value: Project): string[] {
  const errors: string[] = [];
  if (value.name.length < 1 || value.name.length > 256) errors.push("name: length must be between 1 and 256");
  return errors;
}

export function validateMilestone(value: Milestone): string[] {
  const errors: string[] = [];
  if (value.label.length < 1 || value.label.length > 256) errors.push("label: length must be between 1 and 256");
  return errors;
}

export function validateEvent(value: Event): string[] {
  const errors: string[] = [];
  if (value.title.length < 1 || value.title.length > 512) errors.push("title: length must be between 1 and 512");
  return errors;
}

export function validateTask(value: Task): string[] {
  const errors: string[] = [];
  if (value.title.length < 1 || value.title.length > 512) errors.push("title: length must be between 1 and 512");
  return errors;
}

export function validateComment(value: Comment): string[] {
  const errors: string[] = [];
  if (value.body.length < 1 || value.body.length > 10000) errors.push("body: length must be between 1 and 10000");
  return errors;
}

export function validateBugReportRequest(value: BugReportRequest): string[] {
  const errors: string[] = [];
  if (value.title.length < 1 || value.title.length > 512) errors.push("title: length must be between 1 and 512");
  return errors;
}

