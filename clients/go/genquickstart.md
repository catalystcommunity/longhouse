# github.com/catalystcommunity/longhouse/clients/go
...
```sh
go get github.com/catalystcommunity/longhouse/clients/go
...
import (
	...
	api "github.com/catalystcommunity/longhouse/clients/go"
)
...
func main() {
	...
	resp, err := client.Login(context.Background(), api.LoginRequest{SignedAssertion: "example"})
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", resp)
...
func runDatagrams(carrier transport.DatagramCarrier) error {
	req := api.LoginRequest{SignedAssertion: "example"}
	// seq 0 marks an unsequenced datagram.
	datagram, err := transport.NewDatagram(opOrd, 0, api.EncodeLoginRequest(req)).Encode()
	if err != nil {
		return err
	}
	...
	if inbound != nil {
		...
		resp, err := api.DecodeLoginResponse(dg.Payload)
		if err != nil {
			return err
		}
