...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      // The generated TS client lives at repo-root clients/typescript (a real
      // package, @longhouse/client). Alias to its source barrel so Vite
      // transpiles it like any in-tree TS — no separate build step required.
  ...
  server: {
    // clients/ is a sibling of webapp/ (outside the Vite root), so allow the
    // dev server to read it.
